...
<body>
    ...
    <div>
        <table>
            <tr>
                <td class="fontStyle fontBold"> {{ $producteur->name }} </td>
            </tr>
            <tr>
                <td class="fontStyle"> {{ $producteur->email }}</td>
            </tr>
        </table>

        <table style="margin-left: 500px !important; margin-top: -100px !important;">
            <tr>
                <td class="fontStyle fontBold"> Commande n°{{ $commande->id }} </td>
            </tr>
            <tr>
    ...
    <table style="margin-top: 45px !important; margin-left: 10px; width: 100%; padding: 0;" class="fontStyle tableData">
        ...
        <tbody>
        @foreach ($collectionCommandes as $commandesProducts)
        <tr style="outline: 0;">
            <td style="border: 1px solid black; padding-left: 5px;"> {{ $commandesProducts->product->name }}</td> 
            <td style="border: 1px solid black; padding-left: 5px;"> {{ $commandesProducts->quantite }}</td> 
            <td style="border: 1px solid black; padding-left: 5px;"> {{ $commandesProducts->product->prix }}</td> 
