...
<template v-slot:default="props">
			<v-row>
				...
				>
					<v-card elevation="1" rounded="xl">
						...
						<v-divider></v-divider>
						<v-card-title class="subheading font-weight-bold d-flex justify-space-between">
							{{ item.name }} 
							<formProduct :isEdit="true" :product="item" @updateProduct='update($event)'/>
						</v-card-title>
						...
						<v-list dense>
							<v-list-item v-for="(key, index) in filteredKeys" :key="index">
								<v-list-item-content v-if="key != 'image'" :class="{ 'blue--text': sortBy === key }">
									{{ key }}:
								</v-list-item-content>
								<v-list-item-content
									class="align-end"
			...
			<template v-slot:footer>
				<v-row class="mt-12 mb-12" align="center" justify="center">
					...
					<v-btn fab dark color="orange lighten-4" class="ml-1" @click="nextPage">
